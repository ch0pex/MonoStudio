...
if [ ! -d "$WIN_DIR" ]; then
  ...
  powershell.exe -ExecutionPolicy Bypass -NoProfile -Command "& '$VS_DEV_SHELL' -Arch amd64 -HostArch amd64; cd $PROJECT_ROOT; cmake.exe --preset '${PRESET_BASE}-windows-cl'; if (-not \$?) { exit 1 }"
  check_success
fi
...
if [ ! -d "$LIN_CLANG_DIR" ]; then
  ...
  cmake --preset "$PRESET_BASE-linux-clang"
  check_success
fi
...
if [ ! -d "$LIN_GCC_DIR" ]; then
  ...
  cmake --preset "$PRESET_BASE-linux-gcc"
