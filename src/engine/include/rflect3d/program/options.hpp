...
inline std::string
help_message(boost::program_options::options_description const &desc) {
  std::stringstream ss{};
  ss << desc << "\n";
  ss << "##########################\n";
  ss << "#      TOML config       #\n";
  ss << "##########################\n";
  // ss << rfl::toml::write(config::program_example()); TODO
  return ss.str();
}
...
inline err::expected<Config> parse_options(std::span<char *> const args) try {
