...
// --- Includes ---
#include "reflect3d/graphics/core/shader/compiler_detail.hpp"
...
mono::expected<Program>
compile(std::filesystem::path const& shader_path, Stage stage, slang::CompilerOptionName const& options = {}) {
  using namespace slang;
  Slang::ComPtr<IBlob> diagnostic_blob;
  ...
  linked_program->getTargetCode(0, code.writeRef(), diagnostic_blob.writeRef());

  if (diagnostic_blob) {
    LOG_INFO("Linker Diagnostics: {}", static_cast<char const*>(diagnostic_blob->getBufferPointer()));
  }
 ...
 */
// TODO: Add optional argument with compile options like:
// - Search paths (e.g. for #include)
