...
TEST_CASE("One window") {
  ...
  CHECK(window.size() == rflect::Resolution {});
}

TEST_CASE("Two window") {
  rflect::Window window  = rflect::defaults::make_window();
  rflect::Window window2 = rflect::defaults::make_window({.title = "Window2", .resolution {800, 800}});

  CHECK(window.size() == rflect::Resolution {});

  CHECK(window.title() == "Rflect");
  CHECK(window2.title() == "Window2");
  CHECK(window2.size() == rflect::Resolution {800, 800});
}

TEST_CASE("Move constructor and assignment") {
  rflect::Window window = rflect::defaults::make_window({.title = "Window1", .resolution {800, 800}});
